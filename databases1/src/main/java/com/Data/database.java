...
public class database {
    ...
    public static void addCustomer(String ssn_sin, String name, String address){
        try {
            ...
            if(rs.next() == true){
                System.out.println("There already exists an account registered with this SSN/SIN");
                return;
            }
            /**name, address, ssn_sin, date_of_registration, cutomer_id**/
            else{
                st.execute("INSERT INTO customer(name, address, ssn_sin, date_of_registration, customer_id) VALUES ('"+name+"','"+address+"','"+ssn_sin+"','"+registration+"',DEFAULT)");
    ...
    public static void bookRoom(int customer_id, int room_id, String check_in_date, String check_out_date){
        try {
            ...
            st.execute("INSERT INTO booking(booking_id, room_id, customer_id,booking_date,check_in_date,check_out_date, payment, approved) VALUES (DEFAULT,'"+room_id+"', '"+customer_id+"','"+date+"', '"+check_in_date+"','"+check_out_date+"','"+payment+ "',false)");
            st.close();
            db.close();
        } catch (SQLException exception) {
            System.out.println("Exception has been thrown:" + exception.getMessage());
